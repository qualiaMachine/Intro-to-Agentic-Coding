...
judgment, not a substitute (remember the trust episode: incoming PRs are untrusted
input, and prompt injection via PR content is a real pattern).
...
- **MCP (Model Context Protocol)** — connects the agent to external systems: your
  ...
  and treat what flows back through it as untrusted input.
