...
The same structure describes what you can influence. The prompt is the wording of a
single instruction. The context is everything the model sees in its window,
including the project's instruction files. The harness is the system around both.
Each layer contains the one inside it; the later episodes on prompting and context
work at the two inner layers, and the safety episode's limits are set at the
outermost.

![Prompt, context, and harness as three concentric layers around the model.](fig/prompt-context-harness.png){alt='Diagram of concentric circles. Innermost: Model. Around it: Prompt, the wording of a single instruction. Around that: Context, everything the model sees in its context window. Outermost: Harness, the whole system around the model: loop, tools, memory, sandbox. Caption: each layer contains the one inside it.'}
